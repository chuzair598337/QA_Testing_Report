<script setup>
// Lookup-driven icon component — ported from ICON_PATHS in js/app.js
// (legacy static app, lines 15-32). Same SVG path data, same
// viewBox="0 0 24 24".
...
const ICON_PATHS = {
  ...
  // ---- Added in Phase 4 (Dashboard / ReportView) — same Lucide source,
  // same inlined-path convention as the set above. ----
  ...
  // ---- Added in Phase 5 fix pass (mobile nav-toggle hamburger) — same
  ...
  // ---- Added for the typed toast system (error/warning/info/success) ----
...
const svgContent = computed(() => ICON_PATHS[props.name] || '')
</script>

<template>
  <svg class="icon" :class="cls" viewBox="0 0 24 24" aria-hidden="true" v-html="svgContent"></svg>
</template>
